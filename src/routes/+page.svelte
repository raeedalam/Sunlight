<script lang="ts">
	...
	onMount(async () => {
		var idleTimeout: any;
		(document.getElementById('adrbar') as HTMLInputElement).addEventListener('input', () => {
			clearTimeout(idleTimeout);
			var savedInput = (document.getElementById('adrbar') as HTMLInputElement).value;
			idleTimeout = setTimeout(() => {
				if ((document.getElementById('adrbar') as HTMLInputElement).value === '') return;
				if (document.getElementById('autocompleteScript'))
					document.getElementById('autocompleteScript')!.remove();
				if (savedInput === (document.getElementById('adrbar') as HTMLInputElement).value) {
			...
			}, 0); // just run it immediately because your computer is probably fast enough, and if its not, sucks to suck ig
		});
		...
		log();
		await getThemes();
		if (tHs.getThemeFromName(localStorage.getItem('theme'))) {
		...
		document.getElementById('adrbar')!.addEventListener('keydown', (e) => {
			if (e.key === 'Enter') {
				e.preventDefault();
				if ((document.getElementById('adrbar') as HTMLInputElement).value === '') return;
				runService((document.getElementById('adrbar') as HTMLInputElement).value);
				(document.getElementById('adrbar') as HTMLInputElement).value;
			}
		});
		ts = new TabSystem({
			...
			.then((data) => {
				...
				document.querySelectorAll('.abyssFeatGames').forEach((fG) => {
					...
					for (let i = 0; i < data.includeGames.length; i++) {
						fG.appendChild(
							createGameCard(
								data.includeGames[i].name,
								data.includeGames[i].image,
								data.includeGames[i].url
							)
						);
					}
				});
	...
	function addDropElem(name: string) {
...
<div id="settings-temp" style="overflow:auto;display: none;" class="extFrame">
	<center style="overflow:auto">
		...
		<h4>Make sure to test your panic hotkeys. Settings do save.</h4>
		<br />
		<br />
		<div class="settingCards">
			<div class="abyssCard">
				<h1>Theme</h1>
				<br />
				<h2>Select from a list:</h2>
				<div class="settingCardButtons">
					<select id="themeSelect" onchange="themeSwitch(this)">
						<option value="Abyss Default">Default (Dark Abyss)</option>
					</select>
				</div>
			</div>
			<div class="abyssCard">
				<h1>Tab Customize</h1>
				<h2>Tab Title</h2>
				<input type="text" placeholder="Enter Text" onchange="cloakTitle(this.value)" />
				<br />
				<h2>Tab Icon</h2>
			...
			<div class="abyssCard">
				...
				<input
					type="text"
					placeholder="Enter URL (defaults to Google)"
					onchange="cloakURL(this.value)"
				/>
			</div>
		</div>
	</center>
...
<div id="games-temp" style="display: none;" class="extFrame">
	<div class="gameCategoryButtons" />
	<div class="gameCategories" />
</div>

<div id="invalid-temp" style="display: none;" class="extFrame">
	<center>
		<br />
		<br />
		<br />
		<h1>This is an invalid Abyss route, please create a new tab and continue searching.</h1>
	</center>
</div>
...
<header>
	...
	<div id="urlbar">
		<input list="autofill-results" type="url" id="adrbar" placeholder="Type here to search..." />
		<button id="addTab" on:click={createNewTab()}>+</button>
	</div>
	<div id="tabsBar" />
</header>

<datalist id="autofill-results" />
...
<div id="tabTemplate" style="display: none;">
	<div class="tabFrame mainStart">
		...
		<div class="abyssNews">
			<h1>News</h1>
			<p class="abyssNewsTextQW">We updated to </p>
			<hr />
			<h1>Featured Games</h1>
			<p class="abyssFeatGamesText">Loading...</p>
			<div class="abyssFeatGames">Loading...</div>
		</div>
		<div class="adSpot" />
	</div>
</div>

<div class="abyssCard gameVar" id="abyssCardTemplate" style="display: none;">
	<!-- svelte-ignore a11y-img-redundant-alt -->
	<img class="abyssCardImage" alt="Game Image" />
	<!-- svelte-ignore a11y-missing-content -->
	<h1 class="abyssCardTitle" />
	<br />
	<!-- svelte-ignore a11y-missing-attribute -->
	<a class="gameCardButton" rel="nofollow">Play</a>
</div>
